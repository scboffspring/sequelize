  ...
  selectQuery: function(tableName, options, model) {
    ...
    // Else do it the reguar way
      ...
      if (options.groupedLimit) {
        ...
        mainQueryItems.push('SELECT '+(options.distinct ? 'DISTINCT ': '') + mainAttributes.join(', ')+' FROM ('+
        ...
        mainQueryItems.push('SELECT ' + (options.distinct ? 'DISTINCT ': '') + mainAttributes.join(', ') + ' FROM ' + table);
      }
      if (mainTableAs) {
